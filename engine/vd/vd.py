...
class VideoClient():
    def __init__(self, allowed_video_sources: list = None, init_video_clients_cfg: dict = None, clients_threadings: dict = None, requests_overrides: dict = None, apply_common_video_clients_only: bool = False):
        ...
        default_video_client_cfg = dict(
            ...
            work_dir='vd_outputs', freeproxy_settings=None, default_search_cookies={}, default_download_cookies={}, default_parse_cookies={}
        )
    ...
    def processinputs(self, input_tip='', prefix: str = '\n', restart_ui: str = 'startparseurlcmdui'):
        ...
        # quit
        if user_input.lower() == 'q': self.logger_handle.info('Goodbye — thanks for using vd; come back anytime!'); sys.exit()
        # restart
    ...
    '''str'''
    def __str__(self):
        return 'Welcome to use vd!\nYou can visit local engine package for more details.'


'''VideoClientCMD'''
@click.command()
@click.version_option(version=__version__)
@click.option('-i', '--index-url', '--index_url', default=None, help='URL of the video to download. If not specified, vd will start in terminal mode.', type=str, show_default=True)
@click.option('-a', '--allowed-video-sources', '--allowed_video_sources', default=None, help='Platforms to search. Separate multiple platforms with "," (e.g., "AcFunVideoClient,PipixVideoClient"). If not specified, vd will search all supported platforms globally and use the first one that can download the video url.', type=str, show_default=True)
