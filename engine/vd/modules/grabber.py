...
class WebMediaGrabber(BaseVideoClient):
    ...
    '''buildbrowserpage'''
    def buildbrowserpage(self, request_overrides: Optional[dict] = None, cookies_domain: Optional[str] = None):
        ...
        _bp = self.browser_path or DrissionPageUtils.findsystembrowser()
        page = DrissionPageUtils.initsmartbrowser(headless=True, requests_headers=None, requests_proxies=(other_kwargs.get('proxies') or self._autosetproxies()), requests_cookies=cookies, requests_cookies_domain=cookies_domain, co_hook_func=co_hook_func, co_hook_func_args=co_hook_func_args, browser_path=_bp, allow_download=False)
        return page
    '''autoscroll'''
    ...
    def fetchhtmlandmediadrission(self, url: str, request_overrides: Optional[dict] = None) -> Tuple[Optional[str], List[Candidate]]:
        # scope pre-navigation cookies to the target site: DrissionPage cannot set
        # domain-less cookies while the page is still on about:blank
        try:
            from urllib.parse import urlsplit
            _host = (urlsplit(self.safestripurl(url) or url).hostname or '').lower()
            _domain = ('.' + _host[4:] if _host.startswith('www.') else '.' + _host) if _host else None
        except Exception:
            _domain = None
        page = self.buildbrowserpage(request_overrides=request_overrides, cookies_domain=_domain); html: Optional[str] = None; captured: List[Candidate] = []
        try:
